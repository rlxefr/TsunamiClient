...
import static tsunami.features.modules.Module.fullNullCheck;
...
public final class Core {
    ...
    public void onTick(PlayerUpdateEvent event) {
        ...
        for (PlayerEntity p : mc.world.getPlayers()) {
            if (p.isDead() || p.getHealth() == 0)
                TsunamiClient.EVENT_BUS.post(new EventDeath(p));
        }
    ...
    public void drawGps(DrawContext e) {
        if (TsunamiClient.gps_position != null) {
            float dst = getDistance(TsunamiClient.gps_position);
            float xOffset = mc.getWindow().getScaledWidth() / 2f;
            float yOffset = mc.getWindow().getScaledHeight() / 2f;
            float yaw = getRotations(new Vec2f(TsunamiClient.gps_position.getX(), TsunamiClient.gps_position.getZ())) - mc.player.getYaw();
