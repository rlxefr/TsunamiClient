...
import static tsunami.core.manager.client.ConfigManager.IMAGES_FOLDER;
...
public final class ThunderUtility {
    ...
    public static @NotNull String getAuthors() {
        List<String> names = TsunamiClient.MOD_META.getAuthors()
                .stream()
                .map(Person::getName)
                .toList();
    ...
    public static void syncVersion() {
        try {
            if (!new BufferedReader(new InputStreamReader(new URL("https://raw.githubusercontent.com/Pan4ur/THRecodeUtil/main/syncVersion121.txt").openStream())).readLine().equals(TsunamiClient.VERSION))
