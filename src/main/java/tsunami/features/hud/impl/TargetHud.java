...
public class TargetHud extends HudElement {
    ...
    public void onRender2D(DrawContext context) {
        ...
        // If no target, don't render anything
        if (target == null) {
            direction = false;
            return;
        }

        // If animation is too low (HUD is hidden), don't render
    ...
    private void getTarget() {
        // Priority 1: Combat module targets
        if (AutoCrystal.target != null && !AutoCrystal.target.isDead()) {
            target = AutoCrystal.target;
            direction = true;
        } else if (Aura.target != null && Aura.target instanceof LivingEntity && !((LivingEntity) Aura.target).isDead()) {
            target = (LivingEntity) Aura.target;
            direction = true;
        } else if (AutoAnchor.target != null && !AutoAnchor.target.isDead()) {
            target = AutoAnchor.target;
            direction = true;
        }
        // Priority 2: Entity you're looking at
        else if (mc.targetedEntity != null && mc.targetedEntity instanceof LivingEntity && !((LivingEntity) mc.targetedEntity).isDead()) {
            target = (LivingEntity) mc.targetedEntity;
            direction = true;
        }
        // Priority 3: For testing in chat/HUD editor
        else if (mc.currentScreen instanceof ChatScreen || mc.currentScreen instanceof HudEditorGui) {
            target = mc.player;
            direction = true;
        }
        // Priority 4: No target - hide the HUD
        else {
            direction = false;
            // Only clear target after animation finishes to allow smooth fade out
            if (animation.getAnimationd() < 0.02) {
                target = null;
            }
        }
        ...
        if (target != null && target.isDead() && !(mc.currentScreen instanceof ChatScreen || mc.currentScreen instanceof HudEditorGui)) {
            direction = false;
        }
    }
