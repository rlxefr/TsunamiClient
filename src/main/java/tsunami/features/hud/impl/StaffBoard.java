...
public class StaffBoard extends HudElement {
    ...
    public static boolean check(String name) {
        ...
        // Enhanced staff detection - check for the specific roles requested
        if (lowerName.contains("owner") ||
                lowerName.contains("sradmin") ||
                lowerName.contains("admin") ||
                lowerName.contains("srmod") ||
                lowerName.contains("mod") ||
                lowerName.contains("dev") ||
                lowerName.contains("srhelper") ||
                lowerName.contains("helper")) {
            return true;
        }

        // Special handling for mcfunny server
        if (mc.getCurrentServerEntry() != null && mc.getCurrentServerEntry().address.contains("mcfunny")) {
            return lowerName.contains("helper") ||
                    lowerName.contains("moder") ||
                    lowerName.contains("модер") ||
                    lowerName.contains("хелпер");
        }

        // Original detection patterns for other servers (Russian terms)
        return lowerName.contains("curator") ||
                lowerName.contains("куратор") ||
                lowerName.contains("модер") ||
                lowerName.contains("админ") ||
                lowerName.contains("хелпер") ||
                lowerName.contains("поддержка") ||
                lowerName.contains("сотрудник") ||
                lowerName.contains("зам") ||
                lowerName.contains("стажёр");
    }
