...
import tsunami.features.hud.impl.*;
import tsunami.features.modules.misc.*;
import tsunami.features.modules.client.*;
import tsunami.features.modules.combat.*;
import tsunami.features.modules.movement.*;
import tsunami.features.modules.render.*;
import tsunami.features.modules.movement.Timer;
import tsunami.features.modules.player.*;
...
public class ModuleManager implements IManager {
    ...
    public void onLoad(String category) {
        try {
            TsunamiClient.EVENT_BUS.unsubscribe(unHook);
        ...
        modules.forEach(m -> {
            if (m.isEnabled() && (m.getCategory().getName().equalsIgnoreCase(category) || category.equals("none")))
                TsunamiClient.EVENT_BUS.subscribe(m);
        });
    ...
    public void onUnload(String category) {
        modules.forEach(module -> {
            if (module.isEnabled() && (module.getCategory().getName().equalsIgnoreCase(category) || category.equals("none"))) {
                TsunamiClient.EVENT_BUS.unsubscribe(module);
    ...
    public void registerModule(Module module) {
        ...
        if (module.isEnabled())
            TsunamiClient.EVENT_BUS.subscribe(module);
    }
